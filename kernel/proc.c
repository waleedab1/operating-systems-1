    ...
    for(np = proc; np < &proc[NPROC]; np++){
      if(np->parent == p){
        ...
        if(np->state == ZOMBIE){
          ...
          // Task 3
          if(msg_addr != 0 && copyout(p->pagetable, msg_addr, (char *)&np->exit_msg, sizeof(np->exit_msg)) < 0) {
            release(&np->lock);
            release(&wait_lock);
            return -1;
          }
...
// Helper function 1: Finds and returns the min accumulation of Runnable\Running processes
int MinAccumulation(){
  struct proc *pp;
  int min = -1;
  for(pp = proc; pp < &proc[NPROC]; pp++) {
    if(pp->state == RUNNABLE || pp->state == RUNNING){
      ...
      else if(min != -1 && pp->accumulator < min){
        min = pp->accumulator;
      }
    }
  }
...
struct proc* PSPolicy(){
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE){
        ...
        else if(min_proc != 0 && p->accumulator < min_proc->accumulator){
          min_proc = p;
        }
      }
      release(&p->lock);
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    if(p != myproc()){
      ...
      if(p->state == SLEEPING && p->chan == chan) {
        // Task 5 - update accumulation for transition of blocked to ready
        p->accumulator = MinAccumulation();
        p->state = RUNNABLE;
      }
      release(&p->lock);
